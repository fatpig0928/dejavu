...
class SQLDatabase(Database):
    ...
    1) Find duplicates (shouldn't be any, though):
        ...
        from fingerprints
        group by `hash`, `song_id`, `offset` # offset对应于哈希来源的谱图上的时间偏移量
        having cnt > 1
        order by cnt asc;
    ...
    def insert_song(self, songname, file_hash):
        ...
        with self.cursor() as cur:
            ...
            return cur.lastrowid # 最后一个id(自增id)
    ...
    def insert_hashes(self, sid, hashes):
        ...
        for hash, offset in hashes:
            values.append((hash, sid, int(offset)) # 将音频的每一个hash值作为list的一个元素

        with self.cursor() as cur:
            # 经过grouper()处理后的音频数据，被分成n个长度为1000的元组，若分到最后一个，不够分了，用None来代替
            for split_values in grouper(values, 1000): # split_value是一个长度为1000的元组
                # executemany,批量插入，每次插入1000条
                cur.executemany(self.INSERT_FINGERPRINT, split_values)

    def return_matches(self, hashes):
        '''根据hashes查询数据库中匹配的结果
            hashes: 待查询音频的指纹，list=[(hash, offset),(),(),]
        '''
        """
        ...
        mapper = {} # {hash:offset, ....}
        # 每一个音频的一个声道，被分成不同时间段来fingerprint
        for hash, offset in hashes: # 由生成器生成的hashes，需要用for循环来得到每一个值
            mapper[hash.upper()] = offset

        # Get an iteratable of all the hashes we need
        values = mapper.keys() # values 是这个声道的fingerprint的list，每一个时间段offset是一个元素
        ...
        with self.cursor() as cur:
            for split_values in grouper(values, 1000):
                ...
                query = query % ', '.join(['UNHEX(%s)'] * len(li))

                cur.execute(query, li)

                for hash, sid, offset in cur:
                    # (sid, db_offset - song_sampled_offset)
                    # 数据库里的该hash的对应offset和样本里该hash对应的offset的时间差，越相似的音频，这个时间差最多
...
def grouper(iterable, n, fillvalue=None):
    '''将音频的iterable拆解成长度为n的一个个元组，暂时看是为了批量插入数据库用的
    lst = [2,3,4,5,3,6,7,8,10,11]
    a = grouper(lst, 3) # 输出a为[(2, 3, 4), (5, 3, 6), (7, 8, 10), (11, None, None)]
        Args: 
            iterable: list，一首音频的所有数据，[(hash, sid, int(offset)),().....]
            n: 将iterable拆解成长度为n的元组
        Returns:
            将iterable拆解成长度为n的一个个元组组成的list
    '''
    args = [iter(iterable)] * n # 将iterable变成可迭代对象，在转为list
